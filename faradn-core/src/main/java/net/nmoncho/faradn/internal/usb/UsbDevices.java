...
/**
 * Low-level USB discovery and interface/endpoint lookup over {@code javax.usb}.
 * <p>
 * <strong>Not public API.</strong> This lives in an internal package so that
 * {@code javax.usb} types do not leak into the exported surface. Library
 * callers
 * use {@link net.nmoncho.faradn.printer.Devices},
 * {@link net.nmoncho.faradn.transport.UsbTransport} and
 * {@link net.nmoncho.faradn.Printer} instead.
 */
public class UsbDevices {
  ...
  private static final Logger log = LoggerFactory.getLogger(UsbDevices.class);
  ...
  /**
   * Lists connected USB printers as neutral {@link UsbPrinter} values (vendor and
   * product ids), so callers do not need {@code javax.usb} types.
   *
   * @return connected USB printers
   */
  public static List<UsbPrinter> listPrinters() {
    return listPrinterDevices().stream().map(UsbDevices::toUsbPrinter).toList();
  }

  private static UsbPrinter toUsbPrinter(UsbDevice device) {
    final UsbDeviceDescriptor descriptor = device.getUsbDeviceDescriptor();
    return new UsbPrinter(descriptor.idVendor() & 0xFFFF, descriptor.idProduct() & 0xFFFF);
  }
